...
class Instagram {
  ...
  async login({ username, password } = {}) {
    ...
    this.authentication.status = res.body.status === 'ok' ? 'ok' : 'loggedout';
  ...
  _getGraphQLData({ fieldName, queryHash, variables }) {
  ...
  getMediaFeedByUserId({ userId, first = 20, after }) {
    return this._getGraphQLData({
      fieldName: 'edge_owner_to_timeline_media',
      queryHash: '42323d64886122307be10013ad2dcc44',
      variables: {
        id: userId,
        first,
        after
